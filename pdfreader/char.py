class char(object):
    ...
    def __init__(self, xml_char):
        self._font = xml_char.get('font').split('+')[1] if xml_char.get(
            'font') is not None else None
        if self._font is not None:
            (left, top, self._width,
             self._height) = xml_char.get('bbox').split(',')
            ...
            self._isBold = True if len(self._font.split(
                '-')) == 2 and 'Bold' in self._font.split('-')[1] else False
            self._isItalic = True if len(self._font.split(
                '-')) == 2 and 'Italic' in self._font.split('-')[1] else False
