    ...
    def mainIniter(self, argv):
        ...
        for (k, v) in opts:
            if k == '-d': self._debug += 1
                ...
                self._pagenos.update(int(x) - 1 for x in v.split(','))
            elif k == '-m':
                self._maxpages = int(v)
            elif k == '-P':
                self._password = v
            elif k == '-o':
                self._outfile = v
                ...
                self._laparams.all_texts = True
            elif k == '-V':
                self._laparams.detect_vertical = True
            elif k == '-M':
                self._laparams.char_margin = float(v)
            elif k == '-L':
                self._laparams.line_margin = float(v)
            elif k == '-W':
                self._laparams.word_margin = float(v)
            elif k == '-F':
                self._laparams.boxes_flow = float(v)
            elif k == '-Y':
                self._layoutmode = v
            elif k == '-O':
                self._imagewriter = ImageWriter(v)
            elif k == '-t':
                self._outtype = v
            elif k == '-c':
                self._codec = v
    ...
    def run(self):
        ...
        if not self._outtype:
            ...
            if __name__ == '__main__':
                if self._outfile:
                    if self._outfile.endswith(
                            '.htm') or self._outfile.endswith('.html'):
                        self._outtype = 'html'
